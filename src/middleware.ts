...
const retiredPublicRouteRedirects: Array<{
  ...
  { pattern: /^\/combat(?:\/.*)?$/, target: '/gameplay' },
  { pattern: /^\/boss-order(?:\/.*)?$/, target: '/guides' },
  { pattern: /^\/map(?:\/.*)?$/, target: '/guides' },
  { pattern: /^\/steam\/?$/, target: '/download' },
  { pattern: /^\/pc\/?$/, target: '/system-requirements' },
...
export default async function middleware(req: NextRequest) {
  ...
  const productionHosts = new Set([
    'langrisserseaofsword.wiki',
    'www.langrisserseaofsword.wiki',
  ]);
  const canonicalHost = 'www.langrisserseaofsword.wiki';
