...
export const CANONICAL_BASE_URL = 'https://www.langrisserseaofsword.wiki';
...
function getLangrisserBaseUrl(url?: string) {
  ...
  try {
    ...
    if (
      parsedUrl.hostname === 'langrisserseaofsword.wiki' ||
      parsedUrl.hostname === 'www.langrisserseaofsword.wiki'
    ) {
...
export function getBaseUrl(): string {
  ...
  // In production, never let a local or unrelated-project .env value leak into URLs.
  const langrisserBaseUrl = getLangrisserBaseUrl(configuredBaseUrl);
  if (langrisserBaseUrl) {
    return langrisserBaseUrl;
  }
